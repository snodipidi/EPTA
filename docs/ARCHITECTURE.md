...
(`integrations/PythonServiceClient`). Если `PY_*_URL` не заданы, вызовы —
безопасные no-op (возвращают fallback), и монолит полностью работает без Python.
«Включение» сервиса = установка переменной окружения.

Интеграция идёт двумя путями: синхронно по HTTP (с таймаутом) и асинхронно через
очередь (медленный сервис не блокирует запрос). Заготовки будущих сервисов
(`recommendation`, `moderation`, `analytics`, `search`) — в `docker-compose.yml`
под профилем `python`.
